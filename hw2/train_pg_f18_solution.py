...
class Agent(object):
    ...
    def init_tf_sess(self):
        ...
        self.sess.__m  enter__() # equivalent to `with self.sess:`
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            ...
            #----------------------------------My Solution Start-----------------------------#
            sy_logits_na = build_mlp(sy_ob_no, self.ob_dim, "policy_network", self.n_layers, self.size, output_activation=tf.nn.log_softmax)
            #---------------------------------- My Solution End -----------------------------#
            ...
            #----------------------------------My Solution Start-----------------------------#
            sy_mean = build_mlp(sy_ob_no, self.ob_dim, "policy_network", self.n_layers, self.size)
            mean, var = tf.nn.moments(x, axes=[0])
            sy_logstd = tf.log(var)
            #---------------------------------- My Solution End -----------------------------#
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            #----------------------------------My Solution Start-----------------------------#
            z = tf.random_normal(self.ac_dim)
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            ...
            #----------------------------------My Solution Start-----------------------------#
            sy_logprob_n = tf.gather_nd(sy_logits_na, sy_ac_na)
            #---------------------------------- My Solution End -----------------------------#
            ...
            sy_logprob_n = dist.log_prob((sy_ac_na - sy_mean) / sy_logstd)
            #---------------------------------- My Solution End -----------------------------#
    ...
    def sum_of_rewards(self, re_n):
        ...
        #----------------------------------My Solution Start-----------------------------#
        num_tr = len(re_n)
        max_t = max([len(i) for i in re_n])
        
        q_nt = np.zeros((num_tr, max_t))
        gamma_matrix = np.ones((num_tr, max_t))
